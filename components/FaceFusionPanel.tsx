...
interface FaceFusionPanelProps {
  onApplyFaceFusion: (faceRefImage: File) => void;
  isLoading: boolean;
}

const FaceFusionPanel: React.FC<FaceFusionPanelProps> = ({ onApplyFaceFusion, isLoading }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-4 animate-fade-in">
      <p className="text-sm text-center text-zinc-400">Upload an image with a face to blend.</p>
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          {preview ? (
            <>
              <img src={preview} alt="Face reference preview" className="absolute inset-1 w-[calc(100%-0.5rem)] h-[calc(100%-0.5rem)] object-contain rounded" />
              <button
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
      ...
      <p className="text-xs text-center text-zinc-500 mt-1">
        AI will blend the facial features from the uploaded image onto the person in your main photo.
      </p>
    </div>
  );
};

export default FaceFusionPanel;