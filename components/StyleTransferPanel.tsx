...
const StyleTransferPanel: React.FC<StyleTransferPanelProps> = ({ onApplyClothingTransfer, isLoading }) => {
  ...
  const handleApply = () => {
    if (styleRefFile) {
      onApplyClothingTransfer(styleRefFile);
    }
  };
  ...
  return (
    <div className="w-full flex flex-col gap-4 animate-fade-in">
      <p className="text-sm text-center text-zinc-400">Upload an image with clothing to transfer.</p>
      ...
      <div className="flex flex-col gap-4">
        ...
        <button
          ...
          disabled={isLoading || !styleRefFile}
        >
          Apply Clothing Transfer
        </button>
      </div>

      <p className="text-xs text-center text-zinc-500 mt-1">
        AI will dress the person in your main photo with the clothing from the uploaded image.
      </p>
    </div>
  );
};
